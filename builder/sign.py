...
if __name__ == "__main__":
    ...
    # Check if running in integrated builder mode (job archive already extracted)
    integrated_builder = os.environ.get("INTEGRATED_BUILDER") == "1"
    
    if not integrated_builder:
        ...
        print("Integrated builder mode: job archive already extracted, skipping download")
    ...
    # Check if unsigned.ipa already exists (integrated builder mode)
    ...
    if not unsigned_ipa.exists():
        ...
        print("Integrated builder mode: unsigned.ipa already present, skipping download")
