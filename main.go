// LocalSignTools - A simplified local-only version of SignTools
//
// Copyright (C) 2024 LocalSignTools Contributors
//
...
// ---
//
// This project is based on SignTools (https://github.com/SignTools/SignTools)
// Copyright (C) SignTools Contributors
//
// SignTools is licensed under the GNU Affero General Public License v3.0.
// This derivative work maintains the same license and includes modifications
// to create a local-only version with integrated builder support.

package main

import (
	"LocalSignTools/src/assets"
...
func getPublicUrlFatal(provider tunnel.Provider) string {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("failed to obtain public url")
	}
	return serverUrl
}
...
func serve(host string, port uint64) {
	...
	for _, builder := range config.Current.Builder {
		...
		if integrated, ok := builder.(*builders.Integrated); ok {
			// Create adapters to avoid circular imports
			jobAdapter := &storage.JobStorageAdapter{}
			appAdapter := &storage.AppStorageAdapter{}
			integrated.SetProcessJobFn(func() error {
	...
	// Try to start on the specified port, or use a random port if it's already in use
	listener, actualPort, err := server.BindPort(host, port)
	if err != nil {
		log.Fatal().Err(err).Msg("failed to bind port")
	}

	log.Info().Str("address", fmt.Sprintf("%s:%d", host, actualPort)).Msg("starting server")
	log.Fatal().Err(e.Server.Serve(listener)).Send()
}

// getAndHead registers both GET and HEAD handlers for a path
...
func failJob(c echo.Context, job *storage.ReturnJob) error {
	if !storage.Jobs.DeleteById(job.Id) {
		return errors.Errorf("unable to delete return job %s", job.Id)
	}
...
func uploadSignedApp(c echo.Context, job *storage.ReturnJob) error {
	...
	if !ok {
		return errors.Errorf("return job %s appid %s not resolved", job.Id, job.AppId)
	}
	...
	if err != nil {
		return errors.WithMessage(err, "get upload data")
	}
	defer file.Close()
...
func set2FA(c echo.Context, app storage.App) error {
	...
	if !ok {
		return errors.Errorf("no job found for app %s", app.GetId())
	}
...
func uploadUnsignedApp(c echo.Context) error {
	...
	if tweakIds != "" {
		for _, tweakId := range strings.Split(tweakIds, ",") {
			...
			if !ok {
				return errors.Errorf("no tweak upload file with id %s", tweakId)
			}
...
func startSign(app storage.App, builder builders.Builder) error {
	...
	if err != nil {
		return errors.WithMessage(err, "get status url")
	}

	if err := app.SetString(storage.AppWorkflowUrl, statusUrl); err != nil {
		return errors.WithMessage(err, "set workflow url")
	}

	return nil
}

// logErrApp creates a log event with app context
